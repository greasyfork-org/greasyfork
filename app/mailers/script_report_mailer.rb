  ...
  def report_created(report, site_name)
    ...
    text_lambda = lambda { |locale|
      reason_text = case report.report_type
                    when ScriptReport::TYPE_UNAUTHORIZED_CODE then t('mailers.script_report.report_created_offender.reason_unauthorized', locale: locale, script_name: report.script.name(locale), site_name: site_name)
                    when ScriptReport::TYPE_MALWARE then t('mailers.script_report.report_created_offender.reason_malware', locale: locale, script_name: report.script.name(locale), site_name: site_name)
