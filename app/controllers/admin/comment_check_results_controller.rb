module Admin
  class CommentCheckResultsController < BaseController
    ...
    def detail
      params[:from] ||= 30.days.ago.to_date.to_s
      params[:to] ||= Time.zone.today.to_s
      @results = CommentCheckingStats.new(from: params[:from], to: params[:to]).records(strategy: params[:strategy], result: params[:result]).includes(:comment).order('comment.created_at DESC')
