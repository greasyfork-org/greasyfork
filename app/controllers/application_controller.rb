class ApplicationController < ActionController::Base
  ...
  def self.cache_with_log(key, options = {})
    ...
    Rails.cache.fetch(key, options) do
      ...
      o = yield
      Rails.logger.info("Cache stored - #{key} - #{options}") if Greasyfork::Application.config.log_cache_misses
      next o
    end
  end
  ...
  def configure_permitted_parameters
    ...
    devise_parameter_sanitizer.permit(:account_update, keys: [:name, :profile, :profile_markup, :preferred_markup, :locale_id, :author_email_notification_type_id, :show_ads, :show_sensitive, :flattr_username, :filter_locale_default])
  end
