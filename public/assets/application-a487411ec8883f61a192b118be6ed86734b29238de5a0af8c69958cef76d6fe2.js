...
(function() {
	...
	function hookUpAddLocalizedAdditionalInfo() {
		...
		button.addEventListener("click", function(event) {
			...
			xhr.open("get", "/script_versions/additional_info_form" + "?index=" + index);
			xhr.onload = function() {
	...
	function hookUpAddSyncedLocalizedAdditionalInfo() {
		...
		button.addEventListener("click", function(event) {
			...
			var xhr = new XMLHttpRequest();
			xhr.overrideMimeType("text/html");
			xhr.open("get", "/scripts/sync_additional_info_form" + "?index=" + index);
			xhr.onload = function() {
				var frag = document.createElement("div");
				frag.innerHTML = this.responseText;
		...
		if (browserIdLink) {
			browserIdLink.addEventListener("click", function(event) {
				navigator.id.get(function(assertion) {
					...
					if (assertion) {
						location.href = "/auth/browser_id/callback?assertion=" + encodeURIComponent(assertion) + returnToParam;
					} else {
						location.href = "/auth/browser_id/callback?failure=1"+ returnToParam;
					}
				});
				event.preventDefault();
			});
