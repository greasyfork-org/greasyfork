...
class GreasyForkPlugin extends Gdn_Plugin {
  ...
  private function SendNotification($Sender, $IsDiscussion) {
    ...
    $UserInfos = $DiscussionModel->SQL->Select('u.author_email_notification_type_id, u.email, u.name, s.default_name script_name, u.id, ua.UserID forum_user_id')
      ->From('scripts s')
      ->Join('authors a', 's.id = a.script_id')
